...
cv::Mat VisionProcess::CInspectAvi::Binarize(const cv::Mat& src, int dist, int threshold, int minCount, bool findDarker, int blurSize) {
    ...
    // ── x/y 방향 오프셋 (스칼라 경계 체크용) ─────────────────
    const int xOffsets[8] = { 0, 0, -dist, dist, -dist,  dist, -dist,  dist };
    const int yOffsets[8] = { -dist, dist, 0, 0, -dist, -dist,  dist,  dist };

    // ── AVX2 처리 범위 ────────────────────────────────────────
    const int avxStart = dist;                    // AVX 시작 x (이 이전은 스칼라)
    const int avxEnd   = cols - 2 * dist - 32;   // AVX 종료 x (이 이후는 스칼라)

    // ── 스칼라 처리 람다 (경계 skip 포함) ────────────────────
    auto ScalarProcess = [&](int y, int x) {
    ...
    const int tileCount = (rows + TILE_ROWS - 1) / TILE_ROWS; // 전체 rows로 확장

    for (int t = 0; t < tileCount; ++t) {

        const int yStart = t * TILE_ROWS;
        const int yEnd   = std::min(yStart + TILE_ROWS, rows);

        for (int y = yStart; y < yEnd; ++y) {

            const bool bYBorder = (y < dist || y >= rows - dist); // 상단/하단 경계 여부
            ...
            const __m256i vSign     = _mm256_set1_epi8(static_cast<char>(INT8_MIN));
            const __m256i vOne      = _mm256_set1_epi8(1);
#endif

            // 1. 좌측 경계: x = 0 ~ avxStart-1 (스칼라, skip 방식)
            for (int x = 0; x < avxStart && x < cols; ++x) {
                ScalarProcess(y, x);
            }

            // 2. AVX2 내부 구간: x = avxStart ~ avxEnd
            //    단, y가 상단/하단 경계이면 AVX 스킵하고 스칼라로 처리
#ifdef __AVX2__
            if (!bYBorder && avxEnd >= avxStart) {
                const uchar* pSrc = pBase + y * step;
                uchar*       pDst = pDstBase + y * dstStep;

                for (int x = avxStart; x <= avxEnd; x += 32) {
                ...
                // avxEnd 이후 ~ cols-dist-1: 스칼라 (기존 우측 나머지)
                for (int x = avxEnd + 32; x < cols - dist; ++x) {
                    ScalarProcess(y, x);
                }
            }
            else {
                // y가 경계 구간이면 avxStart ~ cols-dist-1 전체를 스칼라로
                for (int x = avxStart; x < cols - dist; ++x) {
                    ScalarProcess(y, x);
                }
            }
#else
            // AVX2 없는 환경: avxStart ~ cols-dist-1 전체 스칼라
            for (int x = avxStart; x < cols - dist; ++x) {
                ScalarProcess(y, x);
            }
#endif

            // 3. 우측 경계: x = cols-dist ~ cols-1 (스칼라, skip 방식)
            for (int x = std::max(avxStart, cols - dist); x < cols; ++x) {
                ScalarProcess(y, x);
            }
        }
    }
